...
envLoader({ path: path.join(ROOT_PATH, '.env') });

function resolveFromProjectRoot(relativeOrAbsolute: string): string {
  return path.isAbsolute(relativeOrAbsolute)
    ? relativeOrAbsolute
    : path.join(ROOT_PATH, relativeOrAbsolute);
}
...
const config = {
  ...
  publish: {
    ...
    policyTemplatePath: resolveFromProjectRoot(
      process.env.POLICY_TEMPLATE_PATH || 'templates/redis/policy.json',
    ),
  },
